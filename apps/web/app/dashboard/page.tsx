...
export default async function DashboardPage() {
  ...
  const stats = [
    ...
    { label: 'Clases hoy', value: (classesToday as number) ?? 0, icon: CalendarDays, color: 'bg-brand-500', border: 'border-l-green-500', show: true },
              ...
              {recentStudents.map((s: any) => {
                ...
                return (
                  <li key={s.id} className="flex items-center gap-4 px-6 py-3">
                    ...
                    <a href={`/dashboard/students/${s.id}`} className="shrink-0 text-xs font-medium text-brand-500 hover:underline">
                      Ver →
                    </a>
                  </li>
                )
