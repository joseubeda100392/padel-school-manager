...
export class PlaytomicClient {
  ...
  async createMatch(opts: {
    ...
    console.error('[pista-viva] payment_intent response:', JSON.stringify(piData))
    ...
    if (method) {
      const patchRes = await fetch(`${CONSUMER_BASE}/v1/payment_intents/${piId}`, {
        ...
        body: JSON.stringify({
          selected_payment_method: method.method_type,
          payment_method_data: method.data ?? undefined,
        }),
      })
