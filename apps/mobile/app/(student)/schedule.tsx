...
export default function StudentScheduleScreen() {
  ...
  async function loadData() {
    ...
    const [{ data: scheds }, { data: bookings }, { data: bag }] = await Promise.all([
      supabase
        .from('schedules')
        .select('*, court:courts(name), coach:users!schedules_coach_id_fkey(name)')
        .eq('is_active', true)
        .order('start_time'),
      supabase.from('bookings').select('schedule_id').eq('student_id', user.id),
  ...
  return (
    <SafeAreaView className="flex-1 bg-gray-50">
      <View className="border-b border-gray-100 bg-white px-4 py-4">
        ...
        <Text className="text-sm text-gray-500">Bolsa: <Text className="font-semibold text-green-600">{bagBalance} clases</Text></Text>
      </View>

      <ScrollView className="flex-1 px-4 py-4">
        {loading && <Text className="text-center text-gray-400">Cargando...</Text>}
        {!loading && schedules.length === 0 && (
          <Text className="mt-8 text-center text-gray-400">No hay clases programadas.</Text>
        )}
