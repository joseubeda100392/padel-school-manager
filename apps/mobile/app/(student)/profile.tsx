...
export default function ProfileScreen() {
  ...
  async function loadProfile() {
    ...
    const [{ data: userData }, { data: history }] = await Promise.all([
      supabase.from('users').select('name, email, current_level_id, currentLevel:levels(name, color, description)').eq('id', authUser.id).single(),
  ...
  const level = (user as any)?.currentLevel
