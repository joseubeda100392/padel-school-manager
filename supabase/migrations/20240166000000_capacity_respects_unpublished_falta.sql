-- Incidente real: una falta con publish_spot=false (el admin la estaba
-- gestionando a mano, para meter a un alumno concreto) SÍ liberaba una plaza
-- en el cálculo genérico de aforo — un alumno la vio y la reservó por
-- autoservicio (book_capacity_spot), y el admin, sin saberlo, metió a otro
-- alumno más por el mismo hueco. Resultado: 5 alumnos en una clase de 4.
--
-- Fix: si ESE día hay alguna falta de un inscrito activo con publish_spot =
-- false, el hueco que abre no cuenta como libre para autoservicio — el
-- admin ya ha dicho que lo gestiona él. (El endpoint de admin para meter
-- alumnos a mano, /api/admin/bookings/spot, ya no tiene este problema
-- porque ahora si aplica su propio chequeo de aforo antes de insertar.)
