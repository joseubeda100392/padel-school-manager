...
### 2.2 Validado con datos reales del club (Vendito Padel) — tres correcciones importantes

**a) El campo correcto para detectar "necesita jugadores" es el número de participantes, no `resource_id` ni `payment_status` solos.** Se probó primero con `resource_id` vacío (hipótesis inicial visual, viendo "Reservar pista" en Manager) y no funcionó: los `OPEN_MATCH` que devuelve `GET /bookings` ya tienen `resource_id` asignado incluso sin estar llenos, porque Playtomic reserva la pista en cuanto hay 2 jugadores (ver 2.1). Después se probó `payment_status !== 'PAID'`, que parecía más robusto (en pago dividido cada jugador paga al apuntarse) — pero también dio falso positivo real: un partido con los 4 jugadores ya apuntados seguía en `PARTIAL_PAID` porque a uno le faltaba terminar de pagar su parte, y eso es un problema de cobro, no de reclutamiento. **La señal correcta es `participant_info.participants.length < 4`** (pádel es 2 contra 2); `payment_status` se conserva solo como información de contexto, no como filtro.

**b) Límite real descubierto — la API no ve los partidos en la fase más temprana.** Comparando la respuesta real de `GET /bookings` con la vista de Manager del club, los partidos abiertos con 0-1 jugador y "Reservar pista" (sin pista aún) **no aparecen en absoluto** en la respuesta de la API — se confirmó con una búsqueda directa por email de un jugador concreto (Gelu) en TODAS las reservas de la ventana, sin filtrar por tipo ni estado: no aparece en ningún sitio. Esto encaja con el comportamiento de 2.1 (la pista no se reserva hasta 2 jugadores): esos partidos no existen todavía como objeto `booking` en el sistema de Playtomic. **Consecuencia:** el motor de reclutamiento (idea A) puede detectar partidos con 2-3 jugadores que necesitan 1-2 más, pero es ciego a los partidos recién creados con 0-1 jugador — que son, irónicamente, los que más necesitan ayuda. No hay forma conocida de solucionar esto con la API de solo lectura; sería necesario evaluar si la API consumer (la que causó el bloqueo) los expone, con el riesgo que eso implica.

**c) Límite operativo de volumen.** El club tiene ~22.000 jugadores históricos en Playtomic (todo el que ha jugado alguna vez en sus pistas, no solo alumnos). Traerlos todos requiere ~224 peticiones secuenciales (paginación por cursor, no paralelizable) y supera el timeout de la plataforma. La importación y la vista previa de jugadores están acotadas a 30 páginas (~3.000) con aviso de "lista incompleta" cuando se corta.
